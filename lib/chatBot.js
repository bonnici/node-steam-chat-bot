...
var ChatBot = function(username, password, options) {
	...
	if(!fs.existsSync('sentryfile.'+this.username+".hash")) {
		logger.info("Sentry file for "+this.username+"doesn't exist");
	} else {
		this.sentryfile = fs.readFileSync('sentryfile.'+this.username+'.hash');
	}
...
ChatBot.prototype.connect = function() {
	...
	if (!this.connected) {
		...
		try {
			if(this.sentryfile) {
				this.steamClient.logOn(this.username, this.password, this.sentryfile, this.sentryfile);
			} else if (this.code) {
				this.steamClient.logOn(this.username, this.password, this.sentryfile, this.code);
			} else {
				this.steamClient.logOn(this.username, this.password, this.sentryfile);
			}
		}
		catch (err) {
